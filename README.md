...
## Neu: Kein BAT mehr nötig

Die Anwendung läuft direkt über `PrintCoverageAnalyzer.exe`.

Zusätzlich kannst du Dateien jetzt sehr einfach übergeben:

- **Drag & Drop auf die geöffnete App** (Dateien ins Fenster ziehen)
- **Dateien direkt auf die EXE ziehen** (Windows übergibt sie als Startargumente)
- oder über den Button "Datei(en) auswählen"

## Papiersorten mit Preis + Auswahl vor Berechnung

- Auswahl der Papiersorte über Dropdown **vor** der Berechnung
...
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\publish-windows.ps1 -Runtime win-x64 -Version 1.2.0
```

Ergebnis:

- `dist/package/PrintCoverageAnalyzer-win-x64-v1.2.0.zip`
...
- Optional: Desktop-Verknüpfung direkt auf die EXE anlegen
...
und eine Desktop-Verknüpfung auf die EXE angelegt.
...
4. Datei(en) auswählen (oder per Drag&Drop übergeben) und auf **„Fläche berechnen & Preis kalkulieren“** klicken
